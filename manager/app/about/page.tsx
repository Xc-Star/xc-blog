...
function getMomentActivities(entries: MomentEntry[]) {
  return entries.map((moment) => ({
    id: `moments-${moment.id}`,
    type: '说说' as const,
    title: moment.content || moment.id,
    date: moment.date ? new Date(moment.date).toISOString() : '1970-01-01T00:00:00Z',
...
export default async function AdminAboutPage() {
  ...
  const posts = getDocActivities(postDocs, 'posts', '文章', 'posts');
  const chatters = getDocActivities(chatterDocs, 'chatters', '杂谈', 'chatter');
  const moments = getMomentActivities(momentEntries);
