...
import { Cloud, X, Hourglass, Download, Link2, Sparkles } from 'lucide-react';
        ...
        >
          ...
          <div className="flex justify-between items-center p-5 border-b border-white/30 dark:border-slate-700/50 bg-white/50 dark:bg-slate-800/50">
            <h3 className="text-sm font-black text-slate-800 dark:text-slate-100 flex items-center gap-2">
              <Cloud size={18} className="text-emerald-500" /> 图床工作台
            </h3>
            <button onClick={onClose} className="w-8 h-8 rounded-full bg-white/50 dark:bg-slate-700/50 flex items-center justify-center text-slate-500 hover:bg-red-500 hover:text-white transition-all cursor-pointer shadow-sm" aria-label="关闭"><X size={16} /></button>
          </div>
            ...
            {!uploadedUrl ? (
              activeTab === 'upload' ? (
                ...
                >
                  ...
                  <div className="text-slate-500 drop-shadow-sm">{isUploading ? <Hourglass size={36} className="animate-pulse" /> : <Download size={36} />}</div>
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button onClick={copyUrlToClipboard} className="py-2.5 rounded-xl bg-white/60 dark:bg-slate-800/60 text-slate-700 dark:text-slate-200 font-bold text-xs hover:bg-white dark:hover:bg-slate-700 transition-all shadow-sm flex items-center justify-center gap-1.5"><Link2 size={14} /> 复制链接</button>
                  <button onClick={() => { onInsert(uploadedUrl); setUploadedUrl(''); setExternalUrl(''); }} className="py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 text-white font-black text-xs shadow-lg shadow-emerald-500/30 hover:from-emerald-600 hover:to-teal-600 transition-all active:scale-95 flex items-center justify-center gap-1.5"><Sparkles size={14} /> 嵌入正文</button>
                </div>
              </div>
            )}
          </div>
