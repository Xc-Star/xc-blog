...
export default function PasswordSection() {
  ...
  return (
    <motion.section initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="bg-white/40 dark:bg-slate-900/40 backdrop-blur-2xl border border-white/50 dark:border-slate-800/50 rounded-[40px] p-8 shadow-2xl">
      <h2 className="text-xl font-black text-slate-800 dark:text-white mb-8">🔐 管理密码</h2>
      <form onSubmit={handleSubmit} className="max-w-xl space-y-5">
        ...
        <button disabled={isSaving} className="w-full py-3 bg-indigo-500 text-white rounded-2xl text-sm font-black shadow-lg hover:bg-indigo-600 shadow-indigo-500/30 transition-all active:scale-95 disabled:opacity-60">
          {isSaving ? '正在更新星钥...' : '✨ 修改管理密码'}
        </button>
      </form>
    </motion.section>
  );
