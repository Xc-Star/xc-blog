...
import { getDocuments, getMoments, getPage } from '../../lib/content.server';

async function getActivities() {
  const [postDocs, chatterDocs, momentsDocs] = await Promise.all([
    getDocuments('post'),
    getDocuments('chatter'),
    getMoments(),
  ]);

  const posts = postDocs.map(({ slug, data }) => ({
    id: `posts-${slug}.md`,
    type: '文章' as const,
  ...
  const chatters = chatterDocs.map(({ slug, data }) => ({
    id: `chatters-${slug}.md`,
    type: '杂谈' as const,
    title: data.title || slug,
    date: data.date ? new Date(data.date).toISOString() : '1970-01-01T00:00:00Z',
  ...
  const moments = momentsDocs.map(({ id, date }) => ({
    id: `moments-${id}.md`,
    type: '说说' as const,
    title: id,
    date: date ? new Date(date).toISOString() : '1970-01-01T00:00:00Z',
    url: `/moments/${id}`
  }));

  return { posts, chatters, moments };
}
...
export default async function AboutPage() {
  ...
  try {
    ...
    if (!page || !page.content.trim()) throw new Error('about.md 不存在或为空');
    // 🌟 改为 let，以便进行文本预清洗
    let content = page.content;
    if (page.cover) coverImage = page.cover;
  ...
  const { posts, chatters, moments } = await getActivities();
