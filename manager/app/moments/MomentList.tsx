...
export default function MomentList({ moments, authorName, avatarUrl }: any) {
  ...
  useEffect(() => {
    let cancelled = false;
    cmsJson<{ success: boolean; moments?: any[]; message?: string }>('/api/moments/list')
      .then(data => {
        if (!cancelled && data.success && Array.isArray(data.moments)) setLiveMoments(data.moments);
      })
      .catch(error => showToast(`说说读取失败：${error.message}`, "error"));
  ...
  const handleDirectPublish = async () => {
    ...
    try {
      ...
      const data = await cmsJson<{ success: boolean; message?: string }>('/api/moments/save', {
        method: 'POST',
  ...
  const executeDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const data = await cmsJson<{ success: boolean; message?: string }>('/api/moments/delete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
