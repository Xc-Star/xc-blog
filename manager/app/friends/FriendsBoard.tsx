...
export default function FriendsBoard() {
  ...
  useEffect(() => {
    let cancelled = false;
    cmsJson<{ success: boolean; data?: Friend[]; message?: string }>('/api/friends/list')
      .then(data => {
        if (!cancelled && data.success && Array.isArray(data.data)) setEditableFriends(data.data);
      })
      .catch(error => showToast(`友链读取失败：${error.message}`, "error"));
