...
function EditorContent() {
  ...
  useEffect(() => {
    const fetchTags = async () => {
      ...
      try {
        const data = await cmsJson<{ success: boolean; postTags?: string[]; chatterTags?: string[] }>('/api/drafts/all_tags');
        if (data.success) {
  ...
  useEffect(() => {
    if (currentDocId !== 'new') {
      const loadDraft = async () => {
        try {
          const data = await cmsJson<{ success: boolean; message?: string; draft: any }>('/api/drafts/get', {
            method: 'POST',
  ...
  const handleSave = async (isPublish: boolean, shouldExitAfterSave: boolean = false) => {
    ...
    setIsSaving(true);
    try {
      const data = await cmsJson<{ success: boolean; message?: string }>('/api/drafts/save', {
