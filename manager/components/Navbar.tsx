...
export default function Navbar() {
        ...
        for (const op of operations) {
          ...
          switch (opAny.type) {
            case 'sync_photowall':
              ...
              body = { albums: opAny.value };
              break;
            case 'sync_friends':
              apiPath = '/api/friends/sync';
              body = { friends: opAny.value };
              break;
            case 'sync_projects':
              apiPath = '/api/projects/sync';
              body = { projects: opAny.value };
              break;
            case 'CONFIG':
              apiPath = '/api/config/update';
              body = { updates: opAny.payload };
              break;
            case 'create_moment':
          ...
          showToast(`🚀 正在请求后端: ${apiPath}`, "info");

          const data = await cmsJson<{ success: boolean; message?: string }>(apiPath, {
            method: 'POST',
  ...
  const executeSyncBlog = async () => {
    ...
    try {
      showToast("🚀 正在刷新博客缓存，请稍候...", "info");

      const data = await cmsJson<{ success: boolean; message?: string }>('/api/sync/execute', {
        method: 'POST',
      });
      if (data.success) {
        showToast(data.message || "✅ 博客缓存已刷新", "success");
      } else {
        showToast(`❌ 刷新失败: ${data.message}`, "error");
      }
    } catch (error) {
      showToast("无法连接到内容刷新引擎", "error");
    }
  };
        ...
        <div className="w-[95%] max-w-7xl mx-auto h-16 flex items-center justify-between px-4 box-border">
          ...
          <div className="flex items-center gap-6" style={{ WebkitAppRegion: 'no-drag' } as React.CSSProperties}>
            ...
            <div className="flex items-center gap-2 ml-2 pl-6 border-l border-slate-300/50 dark:border-slate-600/50">
              <button onClick={handleLogout} title="退出登录" className="w-8 h-8 rounded-xl bg-white/50 dark:bg-slate-800/50 text-slate-500 hover:bg-red-500 hover:text-white flex items-center justify-center transition-all shadow-sm border border-white/20">
                <LogOut size={14} />
              </button>
        ...
        {syncModalOpen && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative w-full max-w-sm bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl rounded-[40px] shadow-2xl border border-white/50 p-10 text-center">
              ...
              <h3 className="text-xl font-black text-slate-900 dark:text-white mb-2">刷新博客缓存</h3>
              <p className="text-sm text-slate-500 mb-8 leading-relaxed text-balance">
                内容已写入共享卷后，确认通知博客端刷新缓存吗？<br />
                <span className="text-xs opacity-80 text-amber-500 font-bold mt-2 block">页面会在后端完成同步后读取最新内容。</span>
              </p>
              <div className="flex gap-3">
                ...
                <button onClick={executeSyncBlog} className="flex-1 py-4 bg-amber-500 hover:bg-amber-600 text-white rounded-2xl text-xs font-black uppercase shadow-lg shadow-amber-500/30 transition-all">确认刷新</button>
              </div>
            </motion.div>
          </div>
        )}
