...
async function getChatterData(slug: string) {
  const entry = await getDocument('chatter', slug);
  if (!entry) notFound();
...
async function getRecentChatters(currentSlug: string) {
  const chatters = await getDocuments('chatter');

  return chatters.map(({ slug, data }) => {
    return { slug, title: data.title || '碎片记录', date: data.date || '1970-01-01' };
