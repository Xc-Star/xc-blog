...
export default async function Home() {
  const postDocs = await getDocuments('post');
  const allPosts: any[] = postDocs.map(({ slug, data, content }) => ({
    slug,
    ...data,
    title: data.title || '',
    ...
    content: content || '',
    date: data.date,
    formattedDate: formatUpdateTime(data.date),
  }));
  ...
  const allChatters: any[] = chatterDocs.map(({ slug, data, content }) => ({
    slug,
    title: data.title || '碎片记录',
    description: data.description || content.substring(0, 60),
    cover: data.cover || 'https://images.unsplash.com/photo-1550751827-4bd374c3f58b?q=80&w=1000&auto=format&fit=crop',
    date: data.date,
    formattedDate: formatUpdateTime(data.date),
  }));
