...
export default function PhotoWallPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    cmsJson<{ success: boolean; data?: Album[]; message?: string }>('/api/gallery/list')
      .then(data => {
        if (!cancelled && data.success && Array.isArray(data.data)) setEditableAlbums(data.data);
      })
      .catch(error => showToast(`图库读取失败：${error.message}`, "error"));
