...
/** 与旧版 gray-matter 输出保持一致的结构，页面里的映射代码因此几乎不用改。 */
...
/** 按发布时间倒序取某一类文档；slug 作为次级排序保证顺序稳定。 */
...
export async function getSiteConfig(): Promise<Record<string, unknown>> {
  ...
  // 数据库缺字段时回落到镜像内置默认值，保证页面永远有东西可渲染
  return { ...(defaultSiteConfig as Record<string, unknown>), ...fromDb };
}

/**
 * 拉取站点配置与结构化数据并写入 globalThis，
 * 供全站（含客户端组件）通过 siteConfig / albums / friendsData / projectsData 代理读取。
 */
